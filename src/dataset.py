...
class Dataset(BaseModel):
    language: Literal[
        "french", "spanish", "swedish", "russian", "portuguese", "polish", "korean", "english", "hindi", "finnish", "turkish", "thai", "japanese", "arabic", "german", "italian", "indonesian", "icelandic", "czech", "chinese",]
    ...
    def get_sentences(
        ...
        for sent in tqdm(
                parsed, desc=f"Extracting sentences for dataset '{self.language}'", leave=False, ):
            sentences.append(
                Sentence(
                    tokens=[token["lemma"] for token in sent], sentence_id=sent.metadata["sent_id"], )
            )
        return sentences
