...
def create_vectorizers(
    language: str, gpu: str | None, input_model: str, output_model: str
) -> tuple[Vectorizer, Vectorizer]:
    return (Vectorizer(
...
def load_data(
    language: str, input_model: Vectorizer, output_model: Vectorizer
) -> tuple[npt.NDArray[np.float64], npt.NDArray[np.float64]]:
    dataset = Dataset(language=language)
    sentences = dataset.get_sentences()

    return (input_model(sentences), output_model(sentences))
